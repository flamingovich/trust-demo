...
const WalletDashboard: React.FC<Props> = ({ assets, totalBalance, walletName, sortOrder, onSortChange, onAction, onRefresh, isRefreshing, formatPrice, t }) => {
  ...
  return (
    ...
    >
      ...
      {/* Balance Card - Dense & Tight */}
      <div className="px-3 mb-4 shrink-0">
        <div className="relative overflow-hidden bg-white dark:bg-dark-surface rounded-[28px] p-6 border border-zinc-200/50 dark:border-dark-border shadow-[0_10px_40px_rgba(0,0,0,0.02)] dark:shadow-none transition-all">
          <div className="relative z-10 flex justify-between items-start">
            <div className="flex-1">
              ...
              <h1 className="text-[34px] font-extrabold tracking-tighter leading-none mb-3 text-black dark:text-white">
                {formatPrice(totalBalance)}
              </h1>
              <div className={`flex items-center space-x-1.5 font-bold text-[13px] ${isPositive ? 'text-green-500' : 'text-red-500'}`}>
                <Triangle 
                  size={10} 
                  fill="currentColor" 
            ...
            <div className="absolute right-[-10px] top-1/2 -translate-y-1/2 flex items-center space-x-[-20px] opacity-[0.06] dark:opacity-[0.12] pointer-events-none">
                <img src="https://cryptologos.cc/logos/tether-usdt-logo.png" className="w-14 h-14 grayscale rotate-[-12deg]" alt="" />
                <img src="https://cryptologos.cc/logos/bitcoin-btc-logo.png" className="w-20 h-20 grayscale rotate-[10deg]" alt="" />
                <img src="https://cryptologos.cc/logos/ethereum-eth-logo.png" className="w-16 h-16 grayscale rotate-[-8deg]" alt="" />
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            <div key={i} className="flex flex-col items-center space-y-1.5">
              ...
              <span className="text-[11px] font-bold text-zinc-500 dark:text-zinc-400 lowercase tracking-tight">{action.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Assets Card Section */}
      <div className="px-3 flex-1 min-h-0 flex flex-col mb-4">
        <div className="flex-1 bg-white dark:bg-dark-surface rounded-[28px] border border-zinc-200/50 dark:border-dark-border shadow-[0_4px_20px_rgba(0,0,0,0.02)] flex flex-col overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 shrink-0 border-b border-zinc-50/50 dark:border-dark-border/30">
            <div className="flex space-x-6">
              <button className="text-[15px] font-extrabold text-zinc-900 dark:text-zinc-100 border-b-2 border-blue-600 pb-1">
                {t.crypto}
              </button>
              <button className="text-[15px] font-bold text-zinc-400 opacity-50 pb-1">NFTs</button>
            </div>
            <div className="flex items-center space-x-1.5">
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar px-3 pb-6 space-y-1">
            {assets.map((asset) => (
              <div 
                key={asset.id} 
                className="flex items-center justify-between p-3 bg-transparent dark:bg-transparent rounded-[22px] hover:bg-zinc-50 dark:hover:bg-dark-elevated/20 active:bg-zinc-100 transition-all cursor-pointer"
                onClick={() => onAction('asset-detail', asset.id)}
              >
                <div className="flex items-center space-x-3.5">
                  <div className="relative shrink-0">
                    <div className="w-[44px] h-[44px] rounded-[14px] bg-zinc-50 dark:bg-dark-bg flex items-center justify-center p-2 border border-zinc-100 dark:border-dark-border/50 shadow-sm">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center space-x-2 mt-0.5">
                      <span className="text-[11px] font-bold text-zinc-400 tracking-tight">{formatPrice(asset.priceUsd)}</span>
                      <span className={`text-[10px] font-extrabold ${asset.change24h >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                ...
                <div className="text-right">
                  ...
                  <p className="text-[12px] text-zinc-400 font-bold opacity-60 mt-0.5">
            ...
            <button className="w-full py-4.5 bg-zinc-50/50 dark:bg-dark-surface/50 rounded-[22px] flex items-center justify-center space-x-2.5 text-zinc-400 font-bold text-[11px] hover:bg-zinc-100 transition-colors uppercase tracking-widest mt-2 border border-dashed border-zinc-200 dark:border-dark-border">
              <Plus size={14} />
              <span>{t.manage}</span>
            </button>
          </div>
