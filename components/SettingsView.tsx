...
import { ChevronLeft, ChevronRight, Shield, Bell, HelpCircle, FileText, Info, Globe, Moon, Sun, Wallet } from 'lucide-react';
...
const SettingsView: React.FC<Props> = ({ onBack, language, onLanguageChange, theme, onThemeChange, t }) => {
  ...
  return (
    <div className="h-full bg-white dark:bg-black text-black dark:text-white flex flex-col pt-12 animate-spring-slide overflow-y-auto no-scrollbar pb-10">
       <div className="flex items-center justify-between px-6 py-4 shrink-0">
        ...
        <h2 className="text-[17px] font-bold">{t.settings}</h2>
      ...
      <div className="px-6 space-y-8 mt-6">
        <div className="flex items-center space-x-4 bg-zinc-50 dark:bg-zinc-900/50 p-6 rounded-[32px] border border-zinc-100 dark:border-white/5 shadow-sm transition-professional">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-zinc-400 text-[10px] font-bold tracking-[0.2em] uppercase">Multi-Coin Wallet</p>
          </div>
          <ChevronRight size={20} className="text-zinc-400" />
        </div>

        <div className="space-y-3">
          <h4 className="text-zinc-400 text-[10px] font-bold uppercase tracking-[0.2em] px-2">{language === 'ru' ? 'ПРИЛОЖЕНИЕ' : 'APP SETTINGS'}</h4>
          <div className="bg-zinc-50 dark:bg-zinc-900/50 rounded-[32px] border border-zinc-100 dark:border-white/5 overflow-hidden transition-professional">
            ...
            >
              ...
              <div className="flex items-center space-x-2">
                <span className="text-sm font-bold text-zinc-400">{theme === 'light' ? t.light : t.dark}</span>
                <ChevronRight size={18} className="text-zinc-300" />
              </div>
            </button>
          </div>
        ...
        <div className="space-y-3">
          <h4 className="text-zinc-400 text-[10px] font-bold uppercase tracking-[0.2em] px-2">{t.support}</h4>
          <div className="bg-zinc-50 dark:bg-zinc-900/50 rounded-[32px] border border-zinc-100 dark:border-white/5 overflow-hidden transition-professional">
            <button className="w-full flex items-center justify-between p-5 border-b border-zinc-100 dark:border-white/5">
              <div className="flex items-center space-x-4">
                <Shield size={20} className="text-green-500" />
                <span className="font-bold text-[15px]">{t.security}</span>
              </div>
              <ChevronRight size={18} className="text-zinc-300" />
            </button>
            <button className="w-full flex items-center justify-between p-5 border-b border-zinc-100 dark:border-white/5">
              <div className="flex items-center space-x-4">
                ...
                <span className="font-bold text-[15px]">{t.notifications}</span>
              </div>
              <ChevronRight size={18} className="text-zinc-300" />
            </button>
            <button className="w-full flex items-center justify-between p-5">
              <div className="flex items-center space-x-4">
                ...
                <span className="font-bold text-[15px]">{t.help}</span>
              </div>
              <ChevronRight size={18} className="text-zinc-300" />
            </button>
          </div>
        ...
        <div className="text-center pt-8 opacity-40">
            <p className="text-[10px] font-bold uppercase tracking-[0.3em]">Version 1.0.4 (Build 88)</p>
        </div>
      </div>
    </div>
  );
