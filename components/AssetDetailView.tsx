...
const AssetDetailView: React.FC<Props> = ({ asset, transactions = [], onBack, onAction, t, language }) => {
  ...
  // Если актив не передан (защита от белого экрана), возвращаемся назад
  useEffect(() => {
    if (!asset) {
      onBack();
    }
  ...
  return (
    <div className="h-full bg-white dark:bg-black text-black dark:text-white flex flex-col animate-ios-slide-in relative transition-colors duration-300">
      <div className="px-6 pt-4 pb-4 flex items-center justify-between shrink-0">
        ...
        <h2 className="text-[17px] font-bold">{asset?.name || 'Token'}</h2>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar">
        ...
        <div className="flex flex-col items-center py-10 px-6">
          ...
          <h1 className="text-4xl font-bold tracking-tight mb-2 text-center">
            {formatValue(balance)} {asset?.symbol}
          </h1>
                ...
                >
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-zinc-500 font-bold uppercase opacity-60">{new Date(tx.timestamp).toLocaleDateString()}</p>
                    </div>
                  </div>
