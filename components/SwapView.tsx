...
const SwapView: React.FC<Props> = ({ assets, onBack, onSwap, t, language }) => {
  ...
  const confirmSwap = () => {
    ...
    // Simulate smart contract execution and DEX routing
    setTimeout(() => {
      const amount = parseFloat(fromAmount);
  ...
  if (isSuccess) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-white dark:bg-black text-black dark:text-white p-10 animate-scale-in">
        <div className="w-24 h-24 bg-blue-500/10 rounded-full flex items-center justify-center text-blue-500 mb-6 shadow-sm">
          <CheckCircle2 size={60} strokeWidth={2.5} className="animate-scale-in" />
        </div>
        <h2 className="text-2xl font-bold tracking-tight">{t.done}!</h2>
  ...
  if (isProcessing) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-white dark:bg-black text-black dark:text-white p-10 animate-fade-in">
        ...
        <p className="text-zinc-500 mt-2 text-center text-sm font-medium opacity-70">Matching liquidity across chains...</p>
      </div>
    );
  }
  ...
  return (
    <div className="h-full bg-white dark:bg-black text-black dark:text-white flex flex-col animate-ios-slide-in relative">
      ...
      <div className="px-6 mt-4 space-y-3 relative flex-1 overflow-y-auto no-scrollbar pb-10">
        ...
        <div className="bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-100 dark:border-white/5 rounded-[32px] p-6 transition-all shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <span className="text-zinc-400 text-[10px] font-bold uppercase tracking-[0.2em]">{language === 'ru' ? 'ПРОДАЕТЕ' : 'YOU PAY'}</span>
            <button 
                onClick={() => setFromAmount(fromAsset.balance.toString())}
                className="text-blue-600 dark:text-blue-500 text-[11px] font-bold uppercase tracking-wider bg-blue-500/5 px-2.5 py-1 rounded-lg border border-blue-500/10"
            >
          ...
          <div className="flex items-center justify-between">
            <button 
              ...
              className="flex items-center space-x-2 bg-white dark:bg-zinc-800 p-2 pr-4 rounded-2xl border border-zinc-100 dark:border-white/5 shadow-sm active:scale-95 transition-all"
            >
            ...
            <input 
              ...
              placeholder="0"
              className="bg-transparent text-right text-3xl font-bold w-1/2 focus:outline-none placeholder-zinc-300 dark:placeholder-zinc-800 tracking-tight"
            />
          </div>
          <p className="text-right text-[11px] text-zinc-400 font-bold mt-2 opacity-60">
        ...
        <div className="flex justify-center -my-6 relative z-10">
          <button 
            ...
            className="w-12 h-12 bg-white dark:bg-zinc-900 border-4 border-white dark:border-black rounded-[20px] flex items-center justify-center text-blue-600 dark:text-blue-500 shadow-xl active:rotate-180 transition-all duration-500 group"
          >
        ...
        <div className="bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-100 dark:border-white/5 rounded-[32px] p-6 transition-all shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <span className="text-zinc-400 text-[10px] font-bold uppercase tracking-[0.2em]">{language === 'ru' ? 'ПОЛУЧАЕТЕ' : 'YOU GET'}</span>
            <span className="text-zinc-400 text-[10px] font-bold uppercase tracking-widest bg-zinc-200/20 dark:bg-zinc-800/50 px-2 py-0.5 rounded-md">Bal: {formatValue(toAsset.balance)}</span>
          </div>
          <div className="flex items-center justify-between">
            <button 
            ...
            <div className="text-right text-3xl font-bold text-zinc-400 dark:text-zinc-500 truncate tracking-tight">
              {toAmount || '0'}
            </div>
          </div>
          <p className="text-right text-[11px] text-zinc-400 font-bold mt-2 opacity-60">
        ...
        <div className="mt-6 px-5 py-4 bg-zinc-50 dark:bg-zinc-900/40 rounded-3xl border border-zinc-100 dark:border-white/5 space-y-3">
          <div className="flex justify-between items-center text-[12px]">
            <span className="text-zinc-500 font-bold uppercase tracking-wider">{language === 'ru' ? 'Курс обмена' : 'Rate'}</span>
            <span className="text-zinc-800 dark:text-zinc-200 font-bold">
              1 {fromAsset.symbol} ≈ {exchangeRate.toFixed(4)} {toAsset.symbol}
            </span>
          </div>
          <div className="flex justify-between items-center text-[12px]">
            <span className="text-zinc-500 font-bold uppercase tracking-wider">{t.networkFee}</span>
            <span className="text-zinc-800 dark:text-zinc-200 font-bold">$1.42</span>
          </div>
        </div>
        ...
        <button 
          ...
          className={`w-full py-5 rounded-[26px] font-bold text-[17px] mt-8 shadow-2xl transition-all ${
        ...
        <div className="absolute inset-0 z-[100] flex items-end justify-center animate-fade-in">
          ...
          <div className="w-full bg-white dark:bg-zinc-950 rounded-t-[44px] h-[85%] flex flex-col p-6 relative animate-ios-bottom-up shadow-2xl border-t border-white/5">
            ...
            <div className="flex items-center justify-between mb-6 shrink-0 px-2">
                <h3 className="text-xl font-bold tracking-tight">{language === 'ru' ? 'Выберите актив' : 'Select Asset'}</h3>
            ...
            <div className="relative mb-6 shrink-0 px-2">
                ...
                <input 
                    ...
                    className="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-100 dark:border-white/5 rounded-2xl py-4 pl-12 pr-4 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all"
                />
            </div>
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar space-y-1 px-2 pb-8">
                {filteredAssets.map(asset => (
                    <button 
                        ...
                        className="w-full flex items-center justify-between p-4 rounded-2xl hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-all active:scale-95 group"
                    >
                        <div className="flex items-center space-x-4">
                            <div className="w-11 h-11 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center group-hover:shadow-md transition-all">
                                <img src={asset.logoUrl} className="w-8 h-8 object-contain" alt="" />
                            </div>
                            <div className="text-left">
                                <p className="font-bold text-base leading-none mb-1">{asset.symbol}</p>
                                <p className="text-[11px] text-zinc-500 font-bold uppercase tracking-wider">{asset.name}</p>
                            </div>
                        </div>
                        <div className="text-right">
                            <p className="font-bold text-[15px]">{formatValue(asset.balance)}</p>
                            <p className="text-[11px] text-zinc-400 font-bold">${formatValue(asset.balance * asset.priceUsd)}</p>
                        </div>
                    </button>
                ))}
            </div>
        ...
        <div className="absolute inset-0 z-[110] flex items-end justify-center animate-fade-in">
          ...
          <div className="w-full bg-white dark:bg-zinc-950 rounded-t-[44px] p-8 pb-12 relative animate-ios-bottom-up shadow-2xl border-t border-white/5">
            ...
            <div className="text-center mb-10 px-4">
              <h3 className="text-2xl font-bold mb-2 tracking-tight">{t.confirm} {t.swap}</h3>
              <p className="text-[12px] text-zinc-500 font-bold uppercase tracking-[0.2em] opacity-60">{t.review}</p>
            </div>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between bg-zinc-50 dark:bg-zinc-900/50 p-6 rounded-[32px] border border-zinc-100 dark:border-white/5 shadow-inner">
                <div className="text-left">
                  <p className="text-zinc-400 text-[10px] font-bold uppercase tracking-widest mb-1 opacity-70">{t.send}</p>
                  <p className="text-xl font-bold tracking-tight">{fromAmount} {fromAsset.symbol}</p>
                </div>
                ...
                <div className="text-right">
                  <p className="text-zinc-400 text-[10px] font-bold uppercase tracking-widest mb-1 opacity-70">{t.receive}</p>
                  <p className="text-xl font-bold text-green-600 tracking-tight">≈ {toAmount} {toAsset.symbol}</p>
                </div>
              </div>

              <div className="px-6 space-y-4">
                <div className="flex justify-between text-[13px]">
                  <span className="text-zinc-500 font-bold uppercase tracking-widest">{t.networkFee}</span>
                  <span className="font-bold text-zinc-800 dark:text-zinc-200">$1.42</span>
                </div>
                <div className="flex justify-between text-[13px]">
                   <span className="text-zinc-500 font-bold uppercase tracking-widest">{language === 'ru' ? 'Проскальзывание' : 'Slippage'}</span>
                   <span className="font-bold text-blue-500">0.5% (Auto)</span>
                </div>
              </div>
